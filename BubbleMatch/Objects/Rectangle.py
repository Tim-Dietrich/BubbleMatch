...
class Rectangle:
    ...
    def get_text_coordinates(self):
        return [self.x_min + 5, self.y_min + 5]

    def get_xml_coordinates(self, image):
        return [
            self.bubble_type,
            self.x_min - self.border_thickness,
            self.y_min - self.border_thickness,
            self.x_max + self.border_thickness,
            self.y_max + self.border_thickness
        ]
